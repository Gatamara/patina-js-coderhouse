...
let obstaculoArray = (obstaculos) => {
  ...
  for (const obstaculo of obstaculos) {
    obstaculosHtml =
      obstaculosHtml + `<span class="span-spot">${obstaculo}</span>`;
  }

  return obstaculosHtml;
};

const creandoCard = document.getElementById("#spots-contenedor");
    ...
    $("#spots-contenedor").prepend(`
            <div style="display: none" class="spot-item ">
                <div class="spot-content p-1">
                    <div class="spot-img">                      
                        <img class="spot-imagen" src="${
                          skatepark.filename
                        }" alt="imagen Skatepark">
                        
                        <img  data-name="${
                          skatepark.name
                        }" class="likes" src="./assets/img/love.png" >
...
const agregarEventoChangeCiudad = () => {
  ...
  $("input[type=checkbox][name=ciudades]").each(function (index) {
    $(this).on("change", () => {
      filtroCiudad = Array.from(checkboxes)
        .filter((i) => i.checked)
        .map((i) => i.value);
...
const agregarEventoChangeObtaculos = () => {
  ...
  $(check).each(function (checkbox) {
    $(this).on("change", () => {
      filtroObstaculo = Array.from(check)
        .filter((i) => i.checked)
        .map((i) => i.value);
...
const aplicarFiltros = () => {
  ...
  if (filtroCiudad.length > 0) {
    spotsFiltrados = spotsFiltrados.filter((spot) =>
      filtroCiudad.includes(spot.city)
    );
  }

  if (filtroObstaculo.length > 0) {
    spotsFiltrados = spotsFiltrados.filter((spot) =>
      spot.obstaculos.some((obstaculo) => filtroObstaculo.includes(obstaculo))
    );
  }

  if (buscarPorNombre != "") {
    spotsFiltrados = spotsFiltrados.filter((spot) =>
      spot.name.toLowerCase().includes(buscarPorNombre)
    );
  }

  if (
    filtroCiudad.length == 0 &&
    filtroObstaculo.length == 0 &&
    buscarPorNombre == ""
  ) {
    mostrarSpots(spots);
  } else {
